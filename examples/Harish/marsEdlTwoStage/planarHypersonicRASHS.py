...
multiPhaseProblem.defineDynamicsFunction(['-D1/mass - mu*sin(gam)/r**2','-D2/mass - mu*sin(gam)/r**2'])
multiPhaseProblem.defineDynamicsFunction(['(v/r - mu/(v*r^2))*cos(gam)','(v/r - mu/(v*r^2))*cos(gam)']) # phase 1

# Define costs (units have to be same for all)
# multiPhaseProblem.pathCost('(alpha^2)+(0.0001*sqrt(rho)*(v**3))','m/s^3')
# multiPhaseProblem.pathCost('(alpha^2)+(0.0001*sqrt(rho)*(v**3))','m/s^3')

multiPhaseProblem.pathCost('(alpha^2)+(v**1)','m/s')
multiPhaseProblem.pathCost('(alpha^2)+(v**1)','m/s')

# Define scalar switching condition for each phase [g1,g2,g3,...,gn]
multiPhaseProblem.switchingConditionSetup(['(-v+vp)']) # phase 1
multiPhaseProblem.switchingConditionSetup(['(v-vp)']) # phase 2
...
ocp.scale(m='h',s='h/v',kg='mass',rad=1,nd=1)
...
ocp.constant('mass',700.0,'kg') # Mass of vehicle, kg
ocp.constant('re',3396200.0,'m') # Radius of planet, m
ocp.constant('Aref',pi*(1.25)**2,'m^2') # Reference area of vehicle, m^2
ocp.constant('vp',300.0,'m/s') # Reference area of vehicle, m^2
ocp.constant('slpRashs',1000,'nd')
ocp.constant('Cd1',1.2,'nd')
ocp.constant('Cd2',14,'nd')
